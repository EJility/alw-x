...
function sendAlert({ symbol, entry, tp, sl, confidence }) {
  const allocation =
    confidence >= 80 ? "100%" : confidence >= 75 ? "85%" : "70%";
  const msg = {
...
app.get("/mock-alert", (_, res) => {
  sendAlert({ symbol: "TEST", entry: 1.23, tp: 1.5, sl: 1.1, confidence: 77 });
...
app.listen(PORT, () => console.log(`ALW-X Sentinel v4.8.1 running on port ${PORT}`));
